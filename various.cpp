...
void various::bf_matrixToQuat(const REAL *matrix,REAL *quat) // convert the 3x3 portion of a 4x4 matrix into a quaterion as x,y,z,w
{
...
void  various::bf_matrixMultiply(const REAL *pA,const REAL *pB,REAL *pM)
{
...
void various::bf_eulerToQuat(REAL roll,REAL pitch,REAL yaw,REAL *quat) // convert euler angles to quaternion.
{
  roll  *= 0.5f;
  pitch *= 0.5f;
  yaw   *= 0.5f;
...
void  various::bf_eulerToQuat(const REAL *euler,REAL *quat) // convert euler angles to quaternion.
{
...
void  various::bf_setTranslation(const REAL *translation,REAL *matrix)
{
...
void  various::bf_transform(const REAL matrix[16],const REAL v[3],REAL t[3]) // rotate and translate this point
{
  if ( matrix )
  {
...
REAL various::bf_dot(const REAL *p1,const REAL *p2)
{
  return p1[0]*p2[0]+p1[1]*p2[1]+p1[2]*p2[2];
}

void various::bf_cross(REAL *cross,const REAL *a,const REAL *b)
{
...
void various::bf_quatToMatrix(const REAL *quat,REAL *matrix) // convert quaterinion rotation to matrix, zeros out the translation component.
{
...
//  q.w = s /2.0f;
//  return q;
void various::bf_rotationArc(const REAL *v0,const REAL *v1,REAL *quat)
{
  REAL cross[3];
...
void various::bf_planeToMatrix(const REAL *plane,REAL *matrix) // convert a plane equation to a 4x4 rotation matrix
{
  REAL ref[3] = { 0, 1, 0 };
  REAL quat[4];
...
void various::bf_inverseRT(const REAL matrix[16],const REAL pos[3],REAL t[3]) // inverse rotate translate the point.
{
...
void  various::bf_rotate(const REAL matrix[16],const REAL v[3],REAL t[3]) // rotate and translate this point
{
  if ( matrix )
  {
...
void various::bf_computeOBB(NxU32 vcount,const REAL *points,NxU32 pstride,REAL *sides,REAL *matrix)
{
...
template <class Type> class EigenVal
{
public:
...
bool various::bf_computeBestFitPlane(NxU32 vcount,
                     const REAL *points,
                     NxU32 vstride,
                     const REAL *weights,
                     NxU32 wstride,
...
void various::bf_computeBestFitOBB(NxU32 vcount,const REAL *points,NxU32 pstride,REAL *sides,REAL *matrix,bool bruteForce)
{
  REAL plane[4];
...
void various::bf_computeBestFitOBB(NxU32 vcount,const REAL *points,NxU32 pstride,REAL *sides,REAL *pos,REAL *quat,bool bruteForce)
{
  REAL matrix[16];
